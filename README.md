...
当前 [#35 交付状态](docs/reports/issue-35-validation.json) 明确区分桩层与真实模型。[八次历史试跑](docs/reports/issue-30-acceptance/) 已保留失败或中断证据，尚无完整通过的双轮，不能据此认定 #30 完成。最新修补会在规划阶段拒绝同一初始数据和输入下互相矛盾的冻结案例；修订既有预期必须沿用原案例名、说明原因并独立确认。单测按文件串行执行，避免候选构建子进程争用资源造成短等待超时。CI 通过 `pnpm test` 和 `pnpm test:e2e` 运行无密钥模型桩，包括六步控制面、浏览器体验、隔离与数据保留。故障夹具只证明确定性修复流程；真实调用中的失败标记为 `natural-flow`，只有内置 Agent 在原预算内生成的修复才可计作真实修复证据。首版仍须“验收可靠、体验完整、真实连续迭代可复现、交付说明一致”四项同时成立。
