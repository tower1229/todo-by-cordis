...
2026-09-07 Agent 设计访谈已收口。Agent 的当前实施依据为 [重构详细设计](06-agent-redesign.md)；本文 M0/M1/M2 内容保留阶段背景，不作为限制新 Agent 只能生成文本工作流的依据。调查、执行进度、候选体验（宿主管理的隔离会话、合成可写数据）与绑定式 apply 已有公开命令与测试验收；体验不是第二套完整应用实例；持续迭代与恢复主线另票。
