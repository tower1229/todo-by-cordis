...
用户已选择：**明亮、精致、富有生命感，像一件好用的创意产品。**

第一版只精做三个体验：日常任务列表、自然语言改造入口、改造前后变化。桌面留足呼吸空间，手机保持单列和清晰的输入/操作；版本、错误、源码默认收纳在详情。主画面不出现治理控制台、权限矩阵或满屏执行日志。

视觉应通过协调的中文排版、细致间距、温润材质、一个有记忆点的原创标识和克制的状态动效成立。完成任务、生成中、候选出现和应用后的过渡要有连续感；动画不能延缓操作。避免预制 SaaS 卡片网格和泛用聊天壳。

首个真实页面就按最终视觉标准制作，代码和工程状态放次级层。功能数量可减，排版、布局与响应式不当作后期装饰。UI 产出必须在浏览器中查看并核验手机宽度；尚未接通模型时明确标注交互预览。
...
M0 通过不等于完整应用、隔离安全或 AI 成功率已被证明。下一步直接进入 M1，同时逐步将实验中成立的接口收敛为正式模块，不照搬实验夹具到产品中。

## 8. 已知边界与后续决策

当前未测 Linux、长期内存、真实模型、浏览器、1000 任务负载和大消息性能。进程冷启动样本很少，不能把它当可靠尾延迟。Node 内建 SQLite 在实验中使用，正式存储驱动在 M1 结合运行环境固定。
