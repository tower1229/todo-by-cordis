...
- **Evolution**：`command / observe / close` 隐藏模型调用、相关上下文、候选构建和验证；loop 实现可替换，记录由外部保存。
...
## 9. M2 实施约束（2026-09-07）

Workspace 提供业务上下文、公开文本字段/动作契约及宿主持有的验收器。Runtime、Evolution、Release 不包含 Todo 字段或状态分支；不拆包。Gemini 驱动与工具执行、持久化、发布控制分离，不预建 Agent 框架。

计划冻结可执行目标、验收规则、稳定插件身份、基础版本和组合修订。确认前只读取和规划；修改需求需新方案。候选 TypeScript 使用项目现有 tsc 构建，在独立 Cordis 子进程中运行宿主保护回归。源码、构建结果、契约版本和证据形成不可变产物。切换前重新核对组合和当前数据，同一 SQLite 事务提交活动版本、发布记录、操作回执与运行结果。
...
M2 不增加普通 Todo 功能、不再改版 UI，不建设市场、模板生成器、多模型调度或远程部署。下一阶段仅推进能力组合与 AI 驱动自身的迭代、修复，复用同一套产物、验证和发布机制。
