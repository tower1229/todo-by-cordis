...
日期：2026-09-07。状态：调查、执行进度、候选体验、绑定式 apply、持续迭代与恢复主线已有公开验收。恢复侧：`activationPending` 区分已提交未开放写入；提交后 abort 不补偿、仅就绪失败补偿；重启时 pending 且启动失败会补偿回旧组合并持久化 `recovery`；A12/A13 含 app 故障注入与浏览器真实撤回保数据。
...
业务验收在 Plan 中表达为 Given/When/Then，并绑定明确的可观察结果。为避免建立新的万能测试 DSL，优先用现有命令/查询契约的输入与结果断言、固定浏览器操作及可见状态断言、固定生命周期检查。由可信检查器解释数据化案例；应用代码候选不能改变已冻结案例。辅助成员业务要求可通过 `memberCases` 冻结（目标成员、动作、初始数据、输入、预期最终数据与拒绝案例），由隔离 Workspace 检查器（`workspace/1`）解释；**不再**对辅助字段使用 `probe-*` 式任意业务输入猜测可调用性，业务正确性仅由冻结案例经 command → beforeCommit → 持久化 → query 比较最终事实承担；结构检查保留成员装载、命令注册与归属、计划未授权命令拒绝等不依赖业务输入的核对。通用冒烟不是唯一 oracle。规划门禁（`AffectedAcceptance`）：新增辅助成员必须在本次计划提交该成员受影响动作的成对冻结案例；升级辅助成员时，省略/空 `memberCases` 仅表示 as-is 继承该成员历史成对案例（无历史基线则阻塞），若提交了案例却未覆盖被升级成员的受影响动作（历史动作 ∪ 本次提交动作）则视为错绑并阻塞。宿主在规划 ready 与候选验证共用同一覆盖结构；候选若注册计划未授权的新命令则拒绝并要求重新规划。无关变更继承历史成员案例；修改已有案例预期仍须 `acceptanceReason` 与独立确认，不能靠省略、改名或换绑悄悄移除旧约束。自迭代辅助成员须经可信构建与受限加载；调查可读 `member-source|contract|acceptance/{pluginId}@{versionId}`，生成期用 `read_member` 读取精确实现后再做最小修改。

模型可以提出案例，但不能仅凭自己生成实现和自测报告认定正确。开始前用户能看到业务目标与验收（方案确认用业务语言展示将检查的行为摘要），系统保留已有回归；新类型行为没有可靠检查器时，应在 Plan 阶段揭示并补齐维护者验证能力，不能以自由生成的测试脚本冒充独立 oracle。

用户主动改变既有业务要求时，展示旧规则与替代规则并确认，建立新验收修订。旧版本记录保留；系统保护约束不随业务需求变化。执行开始后禁止修改已冻结断言，失败必须修改候选而不是改期望。

修复类请求必须绑定故障版本，先复现旧版失败再验证新版通过相同案例，并通过其他保护回归；无法复现记录为 blocked/unreproduced，不输出“已修复”。`reproduce` 与新候选验证共用同一份冻结案例和同一隔离 Workspace 检查器。本次不建设无人值守故障自动发布策略。
