...
在线调度运行时由宿主管理的基础服务承担，不伪装成组合成员：`composition.baseServices` 暴露 `host:online-scheduler`（`kind: host-base`，`interfaceId: schedule.runtime`，状态 `active|stopped|released`），不属于 `members`，普通自迭代不能创建、升级或替换该服务。宿主对该服务执行 `create → bind → stop → release`；业务成员只声明 `schedule.register` 规则，到期触发仍经统一 Workspace `command`（含权限、组合版本、任务版本与提交约束），基础服务不得旁路。正式工作区与候选体验各有独立 Workspace/调度上下文；结束体验、关闭工作区或切换实例时释放对应资源。生产使用真实时钟；测试仅允许在 `Workspace.open({ clock })` 启动时注入可控时钟，同时驱动当前时间与计时器；普通页面、候选与 LLM 无改钟工具；仅测试启动可挂载 `/api/test/clock*`。在线服务停止期间不承诺准点执行。
