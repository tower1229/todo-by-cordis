...
export async function runStabilityBrowserPath(input: {
  ...
  try {
    ...
    updateRun(((await (await submitted).json()) as AssistantSnapshot).run);
    ...
    if (currentRunStatus() === "awaiting-input" && input.clarificationAnswer) {
      await page
      ...
      await page.getByRole("button", { name: "发送需求" }).click();
      updateRun(((await (await answered).json()) as AssistantSnapshot).run);
      await expect
        .poll(
          async () =>
            !latestRun || latestRun.status === "planning"
...
async function exerciseExperience(
  ...
  if (bindings?.tags) {
    ...
    await page
      .getByRole("button", { name: bindings.tags.actionLabel, exact: true })
      .first()
      .click();
    ...
    const inputField = receipt.decision.fields.find(
      (field) => field.key === bindings.tags!.inputKey,
    );
    assert.ok(
      inputField,
    ...
    await page
      .getByLabel(inputField.label, { exact: true })
      .fill("  BrowserTag  ");
  ...
  if (bindings?.counter) {
    ...
    await page
      .getByRole("button", {
        name: bindings.counter.actionLabel,
        exact: true,
      })
      .first()
      .click();
    ...
    assert.equal(saved.status(), 200);
    const task = ((await saved.json()) as CommandResult).task!;
    const value = task.fields[bindings.counter.fieldKey] ?? "";
    assert.ok(value, "体验须写出计数字段");
    assert.equal(Number(value), 1, `首次计数应为 1，实际 ${value}`);
