...
const tagsSource = (lower: boolean) => `export default {
 contribute() { return {uiSlots:[{id:'tag-detail',slot:'task.detail',title:'标签',fields:[{key:'tags',label:'标签'}],actions:[{commandId:'setTags',label:'设标签'}]}],fields:[{key:'tags',label:'标签',type:'text'}],commands:[{id:'setTags',label:'设标签',from:['open','done']}]}; },
