...
export type StabilityEvalManifest = {
  schema: "cordis.stability-eval-manifest/2";
  issue: number;
  parentIssue: number;
  title: string;
  /**
...
export const STABILITY_EVAL_MANIFEST: StabilityEvalManifest = {
  ...
  scenarios: [
    ...
    },
    {
      ...
      expectedOutcomeClass: "accurate-block",
      scoringNote:
        "隔离依赖目录真实移除 @types/node 解析入口，健康对照可解析；宿主 environment 与模型共同观察缺失事实，依赖阻塞应准确。",
      requiresExperience: false,
      requiresApply: false,
      requiresRecoveryEntry: true,
    },
...
export function assertManifestFrozen(manifest: FrozenManifest): void {
  if (!manifest.frozen) throw new Error("评估清单尚未冻结，禁止开始运行");
